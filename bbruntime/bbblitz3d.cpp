...
Entity* bbCreateCapsule(int segs, float height, float radius, Entity* p) {
	...
	MeshModel* capsule = MeshUtil::createCapsule(Brush(), segs, height, radius);

	capsule->getObject()->setCollisionRadii(Vector(radius, height * 0.5f + radius, radius));
	capsule->getObject()->setCapsule(
		Vector(0, -height * 0.5f, 0),  // bottom, point
		Vector(0, height * 0.5f, 0),   // top point
		radius
	);
